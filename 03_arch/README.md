...
В проекте с лекции отключена фоновая подгрузка постов с помощью `WorkManager`'а. Включите её и обновите с учётом того, что в вашем приложении теперь есть `PostRemoteKeyDao` и фиксируются id'шники "последних" загруженных постов.
